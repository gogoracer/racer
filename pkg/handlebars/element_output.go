...
type OutputEventHandlers struct {
...
type ElementOutput struct {
	...
	EventHandlers OutputEventHandlers
...
func OUTPUT_A(attributes OutputAttributes, children ...any) *ElementOutput {
	return &ElementOutput{
...
func OUTPUT_E(eventHandlers OutputEventHandlers, children ...any) *ElementOutput {
	return &ElementOutput{
...
func OUTPUT_AE(attributes OutputAttributes, eventHandlers OutputEventHandlers, children ...any) *ElementOutput {
	return &ElementOutput{
		Attributes:    attributes,
		EventHandlers: eventHandlers,
		Children:      children,
...
func (element ElementOutput) HandlebarElement() {}

func (element ElementOutput) GenerateVDOM() interface{} {
	...
	if element.Attributes.For != "" {
		attrs["for"] = string(element.Attributes.For)
	}
	...
	if len(eventHandlers) == 0 {
		tag := engine.NewTag("output", attrs)
		virtualDOMElement = tag
	} else {
		component := engine.NewComponent("output", attrs)
