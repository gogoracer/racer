...
type TrackEventHandlers struct {
...
type ElementTrack struct {
	...
	EventHandlers TrackEventHandlers
...
func TRACK_A(attributes TrackAttributes, children ...any) *ElementTrack {
	return &ElementTrack{
...
func TRACK_E(eventHandlers TrackEventHandlers, children ...any) *ElementTrack {
	return &ElementTrack{
...
func TRACK_AE(attributes TrackAttributes, eventHandlers TrackEventHandlers, children ...any) *ElementTrack {
	return &ElementTrack{
		Attributes:    attributes,
		EventHandlers: eventHandlers,
		Children:      children,
...
func (element ElementTrack) HandlebarElement() {}

func (element ElementTrack) GenerateVDOM() interface{} {
	...
	if element.Attributes.Default != "" {
		attrs["default"] = string(element.Attributes.Default)
	}
	...
	if element.Attributes.Kind != "" {
		attrs["kind"] = string(element.Attributes.Kind)
	}
	if element.Attributes.Label != "" {
		attrs["label"] = string(element.Attributes.Label)
	}
	...
	if element.Attributes.Srclang != "" {
		attrs["srclang"] = string(element.Attributes.Srclang)
	}
	...
	if len(eventHandlers) == 0 {
		tag := engine.NewTag("track", attrs)
		virtualDOMElement = tag
	} else {
		component := engine.NewComponent("track", attrs)
