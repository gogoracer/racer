...
type MeterEventHandlers struct {
...
type ElementMeter struct {
	...
	EventHandlers MeterEventHandlers
...
func METER_A(attributes MeterAttributes, children ...any) *ElementMeter {
	return &ElementMeter{
...
func METER_E(eventHandlers MeterEventHandlers, children ...any) *ElementMeter {
	return &ElementMeter{
...
func METER_AE(attributes MeterAttributes, eventHandlers MeterEventHandlers, children ...any) *ElementMeter {
	return &ElementMeter{
		Attributes:    attributes,
		EventHandlers: eventHandlers,
		Children:      children,
...
func (element ElementMeter) HandlebarElement() {}

func (element ElementMeter) GenerateVDOM() interface{} {
	...
	if element.Attributes.High != "" {
		attrs["high"] = string(element.Attributes.High)
	}
	...
	if element.Attributes.Low != "" {
		attrs["low"] = string(element.Attributes.Low)
	}
	...
	if element.Attributes.Optimum != "" {
		attrs["optimum"] = string(element.Attributes.Optimum)
	}
	...
	if len(eventHandlers) == 0 {
		tag := engine.NewTag("meter", attrs)
		virtualDOMElement = tag
	} else {
		component := engine.NewComponent("meter", attrs)
