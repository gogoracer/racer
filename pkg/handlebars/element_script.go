...
type ScriptEventHandlers struct {
...
type ElementScript struct {
	...
	EventHandlers ScriptEventHandlers
...
func SCRIPT_A(attributes ScriptAttributes, children ...any) *ElementScript {
	return &ElementScript{
...
func SCRIPT_E(eventHandlers ScriptEventHandlers, children ...any) *ElementScript {
	return &ElementScript{
...
func SCRIPT_AE(attributes ScriptAttributes, eventHandlers ScriptEventHandlers, children ...any) *ElementScript {
	return &ElementScript{
		Attributes:    attributes,
		EventHandlers: eventHandlers,
		Children:      children,
...
func (element ElementScript) HandlebarElement() {}

func (element ElementScript) GenerateVDOM() interface{} {
	...
	if element.Attributes.Async != "" {
		attrs["async"] = string(element.Attributes.Async)
	}
	...
	if element.Attributes.Defer != "" {
		attrs["defer"] = string(element.Attributes.Defer)
	}
	...
	if element.Attributes.Nomodule != "" {
		attrs["nomodule"] = string(element.Attributes.Nomodule)
	}
	...
	if len(eventHandlers) == 0 {
		tag := engine.NewTag("script", attrs)
		virtualDOMElement = tag
	} else {
		component := engine.NewComponent("script", attrs)
