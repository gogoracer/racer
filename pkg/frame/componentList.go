...
func (list *ComponentList) Add(elements ...any) {
	for i := 0; i < len(elements); i++ {
		...
		case engine.Teardowner:
			list.AddItems(v)
		case *engine.EventBinding:
		...
		default:
			if engine.IsNonNodeElement(v) {
				...
				// engine.LoggerDev.Error().
				// 	Str("callers", engine.CallerStackStr()).
...
func (list *ComponentList) RemoveAllItems() {
	for i := 0; i < len(list.ComponentListSimple.items); i++ {
		if td, ok := list.ComponentListSimple.items[i].(engine.Teardowner); ok {
			td.Teardown()
		}
	}
